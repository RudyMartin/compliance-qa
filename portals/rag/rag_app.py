...
def render_browse_systems():
    ...
    with col1:
        ...
        for system_type, system_name in systems.items():
            # Create default info for each system type
            icons = {
                "ai_powered": "🤖",
                "postgres": "🐘",
                "judge": "⚖️",
            ...
            icon = icons.get(system_type, "📚")

            with st.expander(f"{icon} {system_name}"):
                st.write(f"**System Type:** {system_type}")
                st.write(f"**Name:** {system_name}")

                # For now, mark all as available since check_system_availability doesn't exist
                status = "🟢 Available"
    ...
    with col2:
        ...
        if instances:
            for instance in instances:
                with st.container():
                    # Use safe get with defaults for potentially missing keys
                    st.write(f"**{instance.get('name', 'Unknown')}**")
                    st.write(f"Type: {instance.get('type', 'N/A')}")
                    st.write(f"Status: {instance.get('status', 'Unknown')}")

                    # Handle health_score that might not exist
                    health = instance.get('health_score', 0.0)
                    if health:
                        st.write(f"Health: {health:.2f}")

                    # Handle collections_count that might not exist
                    collections = instance.get('collections_count', 0)
...
def render_create_system():
    ...
    systems = rag_delegate.get_available_systems()
    # systems returns a dict of {type: name} where name is a string
    selected_system_name = systems.get(st.session_state.selected_system_type, 'Unknown System')

    col1, col2 = st.columns(2)

    with col1:
        st.write(f"**Creating: {selected_system_name}**")
        st.write(f"Type: {st.session_state.selected_system_type}")
    ...
    with col2:
        ...
        # Show generic capabilities based on system type
        capabilities_map = {
            'ai_powered': ['Advanced NLU', 'Multi-modal processing', 'Context retention'],
            'postgres': ['Vector similarity search', 'SQL integration', 'Scalable storage'],
            'judge': ['Quality assessment', 'Response validation', 'Metric tracking'],
            'intelligent': ['Self-optimizing', 'Pattern learning', 'Adaptive responses'],
            'sme': ['Domain expertise', 'Specialized knowledge', 'Expert validation']
        }
        capabilities = capabilities_map.get(st.session_state.selected_system_type, ['Standard RAG operations'])
        for capability in capabilities:
            st.write(f"• {capability}")

        st.write("**Recommended Use Cases:**")
        use_cases_map = {
            'ai_powered': ['Customer support', 'Document Q&A', 'Knowledge extraction'],
            'postgres': ['Enterprise search', 'Large-scale deployments', 'Production systems'],
            'judge': ['Quality control', 'Response evaluation', 'A/B testing'],
            'intelligent': ['Dynamic content', 'Personalization', 'Learning systems'],
            'sme': ['Expert consultations', 'Technical support', 'Specialized queries']
        }
        use_cases = use_cases_map.get(st.session_state.selected_system_type, ['General Q&A'])
