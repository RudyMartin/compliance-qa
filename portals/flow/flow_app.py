...
# Use the domain/projects path for workflow projects
projects_dir = Path(path_mgr.root_folder) / "domain" / "projects"
...
if 'workflows' not in st.session_state or st.session_state.get('reload_workflows', False):
    ...
    if selected_project == 'global':
        workflow_file = Path(path_mgr.root_folder) / "domain" / "workflows" / "portal_workflows.json"
    else:
...
# Import tab modules - we have v4 tabs available
TABS_AVAILABLE = False

try:
    # Import the v4 tabs that actually exist in this directory
    from t_create_flow_v4 import render_create_tab
    from t_manage_flows_v4 import render_manage_tab
    ...
    st.error(f"Tab modules not found: {e}")
