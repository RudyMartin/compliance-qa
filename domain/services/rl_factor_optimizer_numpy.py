"""
RL Factor Optimizer (NumPy Version)
===================================
...
class RLFactorOptimizerNumPy:
    """Optimizes RL factors for maximum cumulative learning effect (NumPy version)."""
    ...
    def optimize_factors(self, recent_rewards: List[float],
        ...
        reward_variance = np.var(recent_rewards) if len(recent_rewards) > 1 else 1
        avg_latency = np.mean(recent_latencies) if recent_latencies else 1
    ...
    def _compute_factor_gradients(self, reward: float, variance: float):
        ...
        if len(set(epsilon_values)) > 1:  # Need variation to compute correlation
            # Simple correlation as gradient proxy
            epsilon_corr = np.corrcoef(epsilon_values, rewards)[0, 1]
        ...
        if len(set(lr_values)) > 1:
            lr_corr = np.corrcoef(lr_values, rewards)[0, 1]
        ...
        if len(set(temp_values)) > 1:
            temp_corr = -np.corrcoef(temp_values, variances)[0, 1]  # Negative for stability
    ...
    def get_exploration_action(self, q_values: Dict[str, float]) -> str:
        ...
        if np.random.random() < self.factors.epsilon:
    ...
    def _softmax_action_selection(self, q_values: Dict[str, float]) -> str:
        ...
        scaled_values = values / self.factors.temperature

        # Compute softmax probabilities
        exp_values = np.exp(scaled_values - np.max(scaled_values))  # Stability trick
    ...
    def process_feedback(self, feedback_type: str, value: float,
        ...
        # Clip raw value
        value = np.clip(value, -self.factors.reward_clipping, self.factors.reward_clipping)
    ...
    def sample_experiences(self) -> List[Dict]:
        ...
        if self.factors.prioritized_replay:
            ...
            indices = np.random.choice(
                len(experiences),
                size=self.factors.replay_sample_size,
                p=probabilities
            )
            ...
            # Uniform sampling
            indices = np.random.choice(
                len(self.experience_replay_buffer),
                size=self.factors.replay_sample_size,
                replace=False
            )
    ...
    def apply_regularization(self, q_values: Dict[str, float]) -> Dict[str, float]:
        ...
        values_array = np.array(list(regularized_values.values()))
    ...
    def get_optimization_report(self) -> Dict[str, Any]:
        ...
        recent_reward = np.mean([p['avg_reward'] for p in recent])
        older_reward = np.mean([p['avg_reward'] for p in older]) if older else recent_reward
    ...
    def _save_factors(self):
        ...
        try:
            ...
            factors_file = Path(root) / "domain" / "workflows" / "projects" / self.project_id / "rl_factors_numpy.json"
...
def get_rl_optimizer_numpy(project_id: str) -> RLFactorOptimizerNumPy:
    """Factory function to get an RLFactorOptimizerNumPy instance."""
    return RLFactorOptimizerNumPy(project_id)