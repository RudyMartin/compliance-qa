...
class SetupService:
    ...
    def dependency_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check AWS credentials (not CLI, just credentials in settings or env)
            from infrastructure.yaml_loader import get_settings_loader
            settings_loader = get_settings_loader()
            aws_config = settings_loader.get_aws_config()
            results['aws_credentials_configured'] = bool(
                aws_config.get('access_key_id') or os.getenv('AWS_ACCESS_KEY_ID')
            )
